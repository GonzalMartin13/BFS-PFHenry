...
const mapResponse = response.map(us => {
    const {id,origen,destino,dimensiones,servicios,peso,total,status,imagen,dni,UserID} = us;
    return {
        id,origen,destino,dimensiones,servicios,peso,total,status,imagen,dni, UserID
    }
})
  return mapResponse;
};
